...
| M14 | Scoring + weighting | Per-dimension score per candidate via a separate LLM call or model evaluation. Weighted aggregate rank using the venture's `dimension_weights` set. New `dimension_scores` jsonb column on `candidate_companies` (migration 0005 — 0004 is M13's citations). | Pending M13 ship |
