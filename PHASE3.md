...
| **M17** | Evidence-based scoring + ranked table (Stage 6) | Scores each RESEARCHED candidate against the 7 weighted dimensions using its Stage 5 cells as evidence (per-candidate Opus call), computes the P3-D16 weighted aggregate, persists to migration 0005's dormant `dimension_scores` / `aggregate_score` columns, and ranks the comparison-table columns by aggregate (unscored candidates last). Triggered from the table toolbar ("Score & rank"); re-runnable overwrite; no `ventures.status` transitions. See P3-D28. | ✓ Done 2026-07-09 |
...
| **P3-D28** | **Stage 6 scores AFTER cell research, from researched cells** (M17, resuming the deferred P3-D14–D21 scoring path with three deliberate deviations). (a) Evidence base = the candidate's Stage 5 cells, not the M13 rationale+citations (P3-D17 superseded) — rank from facts, not brainstorm-stage guesses. (b) **One Opus call per candidate** (P3-D14 superseded): a candidate's evidence table is ~3–6k tokens, so batching all candidates into one call would dilute attention; per-candidate calls also let unresearched candidates be skipped rather than guessed at (floor: 5 evidenced cells). Cross-candidate calibration is handled by absolute per-dimension anchors in the prompt rather than a shared context. (c) **No `ventures.status` transitions and a fresh budget `run_id` per scoring pass**: scoring is re-runnable derived data (UPDATE in place — the audit trail is `llm_call_logs`, not the columns), status gates would touch the four hand-maintained status lists that have already drifted once, and reusing `current_run_id` would inherit the pipeline's accumulated spend against the per-run cap (the Stage 5 truncation failure mode). P3-D15 (Likert 1–5), P3-D16 (weighted sum), P3-D18 (separate aggregate column), and P3-D19 (all-7-dimensions coverage) carry forward unchanged. | `src/server/stage6-score.ts`, `prompts/stage_6_candidate_scoring.md`, table ordering in `table-data.ts`/`table-viewer.ts`, CSV rank/score columns | 2026-07-09 |
