...
**Status:** M1-M14 complete. M13 (web-augmented candidate brainstorm) shipped 2026-05-19 after one prompt-iteration cycle (P3-D13). M14 (parameter builder — the Y-axis of the competitor comparison table) shipped 2026-05-19 per `parameter_builder.md`. A parallel scoring-against-7-dimensions plan was drafted today (M14_SPRINT_PLAN.md, P3-D14 through P3-D21) but deferred — see banner in that file; migration 0005 columns retained dormant on `candidate_companies`.
**Generated:** 2026-05-14 from `/plan-eng-review`
**Last updated:** 2026-05-19 (M14 parameter builder shipped; scoring plan deferred to future milestone)
...
| **M13** | Web-augmented candidate brainstorm | ✓ Done 2026-05-19 | Exa neural search per `implies_search_for` → bundled evidence → single Opus call → citations jsonb (migration 0004). First run 2026-05-15 (48 candidates, $0.466) missed §6b SPDM coverage on training-data archetypes (Vicor / Atom Power / NVIDIA dropped when Exa surfaced evidence-backed alternatives). Root cause: prompt Constraint 5 covered Direct incumbents but didn't extend to SPDM archetypes. Fix landed 2026-05-19: §6b reframed venture-agnostic (P3-D13), prompt Constraints 4+5 tightened, new SELF-AUDIT step. Re-run 2026-05-19 cleared 6/7 criteria with 46 candidates at $0.466 — full SPDM archetype completeness restored. Criterion 7 (anonymization) flagged ABB Electrification as candidate; expected behavior given full profile anonymization and not pursued (would need a `parent_company` orchestrator field). |
| **M14** | Parameter builder (Y-axis of competitor table) | ✓ Done 2026-05-19 | Generates the Tier 3 dynamic fact-collecting parameters that become the column headers of the competitor comparison table. Tier 1 (universal) + Tier 2 (Innovera 7-dim spine, 21 params) are hardcoded; Tier 3 (10–20 params) is LLM-generated per venture from `substitution_landscape` and `strategic_risks`. Spec at `parameter_builder.md`. Migration 0006 adds `parameter_generation_runs` table + `parameters_generating`/`parameters_ready` statuses. Code shipped as `src/server/stage4-parameters.ts`, `src/types/parameter.ts`, `src/lib/parameters/`, `/ventures/[id]/parameters/` UI. Built in parallel by Harry + Felipe; CEO/eng review on the scoring direction below was off-target and the artifacts are retained as a deferred draft. |
| M14b (deferred) | Per-candidate scoring (deferred draft) | Deferred — see banner in `M14_SPRINT_PLAN.md` | Per-dim Likert score per candidate, weighted aggregate rank. Schema + helper + prompt + 31 tests landed; orchestrator + UI never built. Migration 0005 columns (`dimension_scores jsonb`, `aggregate_score numeric`) remain dormant on `candidate_companies`. P3-D14 through P3-D21 describe this path. Picks up cleanly if a future milestone wants ranks layered onto the parameter-collected facts. |
